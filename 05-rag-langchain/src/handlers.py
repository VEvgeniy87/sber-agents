...
async def send_message_with_retry(message: Message, text: str, max_retries: int = 3):
...
async def handle_message(message: Message):
    ...
    try:
        ...
        success = await send_message_with_retry(message, response)
        if not success:
            # Если не удалось отправить ответ, сообщаем об ошибке
            await message.answer(
                "⚠️ Произошла сетевая ошибка при отправке ответа. "
