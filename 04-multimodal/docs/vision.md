...
│   ├── transcribe.py   # Модуль транскрибации голосовых сообщений (PyTorch)
│   └── transcribe_alt.py # Альтернативный модуль транскрибации (без PyTorch)
├── prompts/
...
2. **handlers.py** - обработка событий
   ...
   - Обработчик голосовых сообщений → транскрибация → извлечение транзакций через LLM → сохранение транзакций → показ ответа + статус + баланс
...
```
Telegram → handlers.py (голосовое сообщение → аудиофайл) → transcribe.py/transcribe_alt.py (транскрибация в текст) →
llm.py (structured output) → OpenRouter/Ollama → llm.py → handlers.py (извлечь транзакции, сохранить в transactions, показать ответ + статус + баланс) → Telegram
```
...
- Транскрибация голосовых сообщений может требовать значительных ресурсов CPU/GPU
