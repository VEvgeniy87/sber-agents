...
async def get_transaction_response_image(
    ...
    try:
        ...
        try:
            ...
            # Исправляем отрицательные значения для расходов
            for transaction in parsed_json["transactions"]:
                if transaction.get("type") == "expense" and transaction.get("amount", 0) < 0:
                    logger.info(f"Fixing negative expense amount: {transaction['amount']} -> {abs(transaction['amount'])}")
